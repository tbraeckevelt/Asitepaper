...
def get_config(path_internal, num_replicas=1):
	...
	label = 'default'
	worker_init  = 'source /pfs/lustrep4/scratch/project_465000315/svandenhaute/psiflow-1.0.0rc0/activate.sh\n'
	...
	provider = SlurmProviderVSC(       # one block == one slurm job to submit
		...
		min_blocks=1,           # always keep at least three block open
		max_blocks=50,          # do not use more than fifty blocks
	...
	provider = SlurmProviderVSC(       # one block == one slurm job to submit
		...
		min_blocks=0,           # always keep at least three block open
	...
	provider = SlurmProviderVSC(       # one block == one slurm job to submit
		...
		max_blocks=20,           # do not use more than one block
		walltime='71:59:00',    # walltime per block
		exclusive=False,
		worker_init=worker_init,
		)
