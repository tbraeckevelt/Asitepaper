    ...
    for ens, sim in sim_dct.items():
        ...
        for phase in ['gamma', 'Csdelta', 'FAdelta']:
            ...
            for j, T_NVT in enumerate(T_lst):
                fec = fec = Free_energy_contribution.from_p(Path_phase / sim / str("fec_ref_T" + str(T_NVT) +".pickle"))
                ...
                fec_npt_corr.volumes *= fec_npt_corr.n_fu*molmod.units.angstrom**3
                npt_corr[phase][str(T_NVT)] = fec_npt_corr.calculate_prop()[0] / molmod.units.kjmol
        ...
        for phase, color in {'Csdelta': 'tab:blue', 'FAdelta': 'tab:orange'}.items():
            ...
            for label, f in f_dct[phase]['ave'].items():
                ...
                if label == 'combined':
                    ...
                    plt.arrow(int(label), diff_f[npt_ind]-df_nptcorr, 0, df_nptcorr, head_width=5, head_length=0.2, fc=color, ec=color, alpha=1.0)
                    tel += 1
