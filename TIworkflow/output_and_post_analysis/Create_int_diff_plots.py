    ...
    for ens, sim in sim_dct.items():
        ...
        for phase in ['gamma', 'Csdelta', 'FAdelta']:
            ...
            for j, T_NVT in enumerate(T_lst):
                ...
                low_corr[phase][str(T_NVT)] = fec_low_corr.calculate_prop(0.0) / molmod.units.kjmol
                high_corr[phase][str(T_NVT)] = fec_high_corr.free_energy / (fec_high_corr.n_fu * molmod.units.kjmol)
        ...
        for phase, color in {'Csdelta': 'tab:blue', 'FAdelta': 'tab:orange'}.items():
            ...
            for label, f in f_dct[phase]['ave'].items():
                ...
                if label == 'combined':
                    ...
                    df[tel, :] = diff_f
                    df_lowcorr = low_corr[phase][label] - low_corr[ref_phase][label]
                    df_highcorr = high_corr[phase][label] - high_corr[ref_phase][label]
                    if label == str(T_lst[0]):
                        alpha_l = 1.0
                        alpha_h = 0.2
                    elif label == str(T_lst[-1]):
                        alpha_l = 0.2
                        alpha_h = 1.0
                    else:
                        alpha_l = 0.2
                        alpha_h = 0.2
                    plt.arrow(155, diff_f[0] - df_lowcorr, 0, df_lowcorr, head_width=5, head_length=0.2, fc=color, ec=color, alpha=alpha_l)
                    plt.arrow(595, diff_f[-1], 0, df_highcorr-diff_f[-1], head_width=5, head_length=0.2, fc=color, ec=color, alpha=alpha_h)
                    tel += 1
